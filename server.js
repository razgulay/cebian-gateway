...
//   Worker → extension : { kind: 'sendMessage_result', request_id, ok,
//                          message_id?, error? }
//
...
wss.on('connection', (ws) => {
  ...
  ws.on('message', async (raw) => {
    ...
    if (!data || data.kind !== 'sendMessage' || !data.chat_id || typeof data.text !== 'string') {
      return;
    }
    ...
    try {
      ws.send(JSON.stringify({ kind: 'sendMessage_result', request_id: data.request_id, ...result }));
    } catch {
